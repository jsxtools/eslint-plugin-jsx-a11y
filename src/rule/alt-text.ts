...
const ariaLabelHasValue = (prop: any) => {
...
const ruleByElement: Record<string, {
  (context: Rule.RuleContext, node: JSXOpeningElement, nodeType: string, elementType: string): any
}> = {
...
export default {
  ...
  create: (context: any) => {
    const options: { elements?: string[] } & Record<string, any> = context.options[0] || {};
    ...
    const customComponents = elementOptions.flatMap(
      (element) => options[element] as string,
    );
